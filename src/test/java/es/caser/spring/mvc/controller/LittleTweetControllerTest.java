...
public class LittleTweetControllerTest {
	...
	public void shouldShowRecentTweets() throws Exception {
		...
		MockMvc mockMvc = standaloneSetup(controller)
				.setSingleView(new InternalResourceView("/WEB-INF/views/tweets.jsp")).build();
		/**
		 * infiere el nombre del tipo y la coleccion
		 */
		mockMvc.perform(get("/tweets").param("max", "" + Long.MAX_VALUE).param("count", "20"))
				.andExpect(view().name("tweets")).andExpect(model().attributeExists("littleTweetList"))
	...
	public void should_ShowRightTweet_whenUsingId() throws Exception {
		...
		mockMvc.perform(get("/tweets/1")).andExpect(view().name("tweet"))
